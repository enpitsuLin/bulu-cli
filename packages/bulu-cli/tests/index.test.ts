...
import { createL1ActionHash } from '../src/protocols/hyperliquid/signing'
...
test('createL1ActionHash matches manual implementation', () => {
  ...
  const expiresMarker = new Uint8Array([])
  const manualHash = `0x${bytesToHex(keccak_256(concatBytes(actionBytes, nonceBytes, vaultMarker, new Uint8Array(), expiresMarker, new Uint8Array())))}`
...
test('createL1ActionHash with vault address', () => {
  ...
  const expiresMarker = new Uint8Array([])
  const manualHash = `0x${bytesToHex(keccak_256(concatBytes(actionBytes, nonceBytes, vaultMarker, vaultBytes, expiresMarker, new Uint8Array())))}`
